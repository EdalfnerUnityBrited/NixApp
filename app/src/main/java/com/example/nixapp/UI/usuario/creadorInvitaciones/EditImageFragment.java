...
public class EditImageFragment extends BottomSheetDialogFragment implements SeekBar.OnSeekBarChangeListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        seekBar_brightness = (SeekBar) itemView.findViewById(R.id.seekbar_brillo);
        seekBar_contrast = (SeekBar) itemView.findViewById(R.id.seekbar_contraste);
        seekBar_saturation = (SeekBar) itemView.findViewById(R.id.seekbar_saturacion);
    ...
    public void onProgressChanged(SeekBar seekBar, int i, boolean b) {
        if (listener != null){
            if (seekBar.getId() == R.id.seekbar_brillo){
                listener.onBrightnessChanged(i-100);
            }
            else if (seekBar.getId()==R.id.seekbar_contraste){
                i+=10;
                float value = .10f*i;
                listener.onContrastChanged(value);
            }
            else if (seekBar.getId()==R.id.seekbar_saturacion){
                float value = .10f*i;
